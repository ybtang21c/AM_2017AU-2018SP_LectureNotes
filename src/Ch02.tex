...
% 	  法线方程。
	  ...
	  \item 讨论下列函数在$x=0$处的连续性、可导性以及导函数的连续性
	  \begin{enumerate}[(1)]
	    ...
	    \end{array}\right.$
	  \end{enumerate}
	  \item （选作）已知函数$g(x)$在$x=a$连续，问函数$f(x)=|(x-a)|g(x)$
...
\begin{thx}
	设$y=f(x)$和$x=g(y)$互为反函数，若$f(x)$可导，且$f'(x)\ne 0$，则
	$$g'(y)=\df1{f'(x)}.$$
\end{thx}
...
\begin{ext}
	...
	\begin{enumerate}  
	  \item 同济教材习题2-2：7，8
	  \item 自行完成不少于100道各种求导计算题（{\it\b 可不写在作业本上}）。
...
\begin{ext}
	...
	\begin{enumerate}  
	  ...
	  \begin{enumerate}
	    ...
	    \item $y=x\ln x$。
	  \end{enumerate}
	  \item 已知$f(x)=x^2\ln(1+x)$，求$f^{(n)}(0)$。
	\end{enumerate}
\end{ext}
...
\begin{ext}
	...
	\begin{enumerate}  
	  ...
	  \end{enumerate}
	  \item 统计教材习题2-5：4
	  \item 设$a>0$，$|x|<<a$（表示$|x|$远远小于$a$），证明近似公式
